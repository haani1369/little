...
def variable_renamer(given_string):
    ...
    variable_dictionary = {}
    special_cases = {"typedef","unsigned","int","bool","char", "const",
                     "static", "static int", "static bool", "void",
                     "false", "true", "for", "while", "rand"}
    index = 0
    new_string = ""
    ...
    for section in split_code:
            ...
            if(index%2==0):  
                ...
                idk = lambda x: "\\" + hex(ord(x))[1:]
                section = "".join(map(idk, section))
...
def whitespace_remover(a):
    ...
    for s in splits:
            ...
            if(index%2==0):                
                s_spaceless = (" ".join(s.split())).replace("\t", " ").replace("\n", " ")
    ...
    for filename in os.listdir(cwd):
        ...
        if(".c" in filename or ".h" in filename):
            with open(os.path.join(cwd, filename)) as file_data:
                ...
                print("PASS\n")
                header, file_string = build_header(file_string)
